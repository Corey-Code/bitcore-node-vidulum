...
var WebService = function(options) {
  ...
  this.disablePolling = options.disablePolling || false;
...
WebService.prototype.start = function(callback) {
  ...
  if (this.disablePolling) {
    this.io = socketio.listen(this.server, {transports: ['websocket']});
  } else {
...
WebService.prototype._getOrigin = function(socket) {
  ...
  if (typeof socket.handshake.headers.origin !== "string") {
    return "";
  }
...
WebService.prototype.socketHandler = function(socket) {
  ...
  var origin = this._getOrigin(socket);
  if (!(this.node.allowedOriginRegexp.test(origin))) {
    socket.send('Origin ' + origin + ' not allowed.');
  ...
  if (this.enableSocketRPC) {
    if (this.node.messageLog) {
        socket.on("message", this.socketMessageHandlerWithMessageLog.bind(this));
      } else {
...
WebService.prototype.messageLogCounter = 0;
 WebService.prototype.messageLogger = function (str) {
  fs.appendFile(this.node.messageLog, str + '\n', function (err) { });
}
 WebService.prototype.socketMessageHandlerWithMessageLog = function (message, socketCallback) {
  var self = this;
  var counter = self.messageLogCounter++;
  self.messageLogger(JSON.stringify({ "id": counter, "req": message }));
  var start = process.hrtime();
  return self.socketMessageHandler(message, function (response) {
    var diff = process.hrtime(start);
    self.messageLogger(JSON.stringify({ "id": counter, "et": diff[0] * 1e9 + diff[1], "res": response }));
