...
Bitcoin.prototype.subscribeBalance = function(emitter, addresses) {
  var self = this;
   ...
   log.info(emitter.remoteAddress, 'subscribe:', 'bitcoind/addressbalance', 'total:', _.size(this.subscriptions.balance));
};

Bitcoin.prototype.unsubscribeBalance = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
   ...
   log.info(emitter.remoteAddress, 'unsubscribe:', 'bitcoind/addressbalance', 'total:', _.size(this.subscriptions.balance));
};

Bitcoin.prototype.unsubscribeBalanceAll = function(emitter) {
...
Bitcoin.prototype._loadSpawnConfiguration = function(node) {
  ...
  $.checkArgument(this.options.spawn, 'Please specify "spawn" in vidulumd config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in vidulumd config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in vidulumd config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './vidulum.conf');

  log.info('Using vidulum config file:', configPath);
...
Bitcoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to vidulumd from bitcore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from vidulumd. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from vidulumd. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in vidulum.conf'
  );

  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that vidulumd is undergoing a reindex. ' +
...
Bitcoin.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/vidulum.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/vidulum.conf';
    }
  }
  return networkPath;
};
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Vidulum Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Bitcoin.prototype._notifyBalanceSubscribers = function(txid, transaction) {
 ...
 Bitcoin.prototype._notifyBalanceSubscriber = function (address, txid) {
    var self = this;
        ...
        if(err) {
         ...
         if (emitters) {
            for(var j = 0; j < emitters.length; j++) {
                emitters[j].emit('bitcoind/addressbalance', {
                    address: address,
                    txid: txid,
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('Vidulum Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
...
Bitcoin.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/vidulumd.pid';
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned vidulum process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean vidulum process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
Bitcoin.prototype._spawnChildProcess = function(callback) {
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    log.info('Starting vidulum process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Vidulum process unexpectedly exited with code:', code);
        log.warn('Restarting vidulum child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Vidulum process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn vidulumd.'));
      }
...
Bitcoin.prototype._connectProcess = function(config, callback) {
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to vidulumd.'));
    }
...
Bitcoin.prototype.start = function(callback) {
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Vidulum configuration options "spawn" or "connect" are expected'));
    }
...
+ * @param {String|Address|Array} addressArg - An address string, ravencore address, or array of addresses
+ * @param {Object} options
+ * @param {Function} callback
+ */
Bitcoin.prototype.getAddressesMempoolBalance = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
   self.client.getAddressMempool({addresses: addresses}, function(err, response) {
      if (err) {
...
Bitcoin.prototype.getJsonBlock = function(blockArg, callback) {
  var self = this;
   function queryBlock(err, blockhash) {
      if (err) {
          return callback(err);
...
+ * @param {Function} callback
+ */
Bitcoin.prototype.getJsonRawTransaction = function(txid, callback) {
  var self = this;
  ...
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('vidulumd spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('vidulumd process did not exit'));
      }
...
Bitcoin.prototype.getBlockReward = function(height) {
 ...
 * @param {Function} callback
 */
Bitcoin.prototype.listUnspent = function(minConf, maxConf, addresses, callback) {
  var self = this;
 ...
 * @return {*}
 */
Bitcoin.prototype.getMiningInfo = function(callback) {
  var self = this;
...
Bitcoin.prototype.getNetworkHash = function(height, callback) {
  var self = this;
...
Bitcoin.prototype.getPeerInfo = function(callback) {
  var self = this;
